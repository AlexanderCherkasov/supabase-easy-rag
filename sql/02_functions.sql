    ...
    IF p_kb_token IS NULL OR btrim(p_kb_token) = '' THEN
        ...
        VALUES (
            ...
            jsonb_build_object(
                ...
                'actor_role', COALESCE(current_setting('request.jwt.claim.role', true), 'service_role')
            )
        );
        RAISE EXCEPTION 'Knowledgebase token is required';
    END IF;
...
AS $$
BEGIN
    PERFORM knowledgebase.assert_retrieval_access(p_kb_token);
...
-- 3. Hybrid Search RPC (Weighted Vector + FTS + Title Boost)
